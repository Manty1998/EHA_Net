...
parser.add_argument('--batch_size', type=int, default=24, help='batch_size per gpu')          #default=24
...
# label and unlabel
parser.add_argument('--labeled_bs', type=int, default=12, help='labeled_batch_size per gpu') #default=12
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # 2先粘贴后和谐化
            net_input_unl0 = uimg_a * img_mask + img_a * (1 - img_mask)
            net_input_unl = adain2(net_input_unl0, uimg_b)

            # 3先和谐化后粘贴
            # net_input_unl0 = adain2(uimg_a, img_a)
            # net_input_unl = net_input_unl0 * img_mask + img_a * (1 - img_mask)

            # 1 原论文
            # net_input_l = img_b * img_mask + uimg_b * (1 - img_mask)

            # 2 先粘贴后和谐化
            net_input_l0 = img_b * img_mask + uimg_b * (1 - img_mask)
            net_input_l = adain2(net_input_l0, img_a)

            # 3先和谐化后粘贴
            # net_input_l0 = adain2(img_b, img_a)
            # net_input_l = net_input_l0 * img_mask + uimg_b * (1 - img_mask)
