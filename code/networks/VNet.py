...
import pdb
from .AbSViT3D import  Attention3D
...
class Encoder(nn.Module):
    def __init__(self, n_channels=16, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False):
        super(Encoder, self).__init__()
        self.has_dropout = has_dropout
        convBlock = ConvBlock if not has_residual else ResidualConvBlock

        # # 添加 Swin Transformer 模块
    ...
    def forward(self, input):
        ...
        # # 在每个编码块之后加入 Swin Transformer 模块
        # if self.swin_transformer is not None:
        ...
        # # 在每个编码块之后加入 Swin Transformer 模块
        # if self.swin_transformer is not None:
        #     x3 = self.swin_transformer(x3)
        x3_dw = self.block_three_dw(x3)

        x4 = self.block_four(x3_dw)
        # # 在每个编码块之后加入 Swin Transformer 模块
        # if self.swin_transformer is not None:
        #     x4 = self.swin_transformer(x4)
        x4_dw = self.block_four_dw(x4)

        x5 = self.block_five(x4_dw)
        # # 在每个编码块之后加入 Swin Transformer 模块
        # if self.swin_transformer is not None:
        #     x5 = self.swin_transformer(x5)
        if self.has_dropout:
            x5 = self.dropout(x5)
...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        self.attention = Attention3D(dim=256, num_heads=8)  # 假设Attention类已定义
        # # 添加新参数
        # self.qkv_weight = nn.Parameter(torch.randn((256, 256)))
        # self.proj_weight = nn.Parameter(torch.randn((256, 256)))
        # self.proj_bias = nn.Parameter(torch.zeros(256))
        dim_in = 16
        feat_dim = 32
    ...
    def forward(self, input):
        ...
        # # 应用注意力机制于编码器的最后一个特征
        features[-1] = self.attention(features[-1])
