...
def adain2(content_feat, style_feat):
...
def calc_mean_std(feat, eps=1e-5):
    ...
    feat_mean = feat.contiguous().view(N, C, -1).mean(dim=2).view(N, C, 1, 1, 1)
    return feat_mean, feat_std
...
def get_foreground_mean_std(features, mask, eps=1e-5):
    ...
    mask = mask.unsqueeze(0)  # 添加批大小维度
    mask = mask.unsqueeze(1)  # 添加通道维度
    region = features * mask
    sum = torch.sum(region, dim=2)  # 在空间维度上求和 (B, C, spatial_dim)
    num = torch.sum(mask, dim=2)  # 在空间维度上求和 (B, C, spatial_dim)
    mu = sum / (num + eps)
    mean = mu[:, :, None]  # 添加一个维度 (B, C, 1)

    # 计算方差和标准差
    var = torch.sum((region + (1 - mask) * mean - mean) ** 2, dim=2) / (num + eps)
    var = var[:, :, None]  # 添加一个维度 (B, C, 1)
    std = torch.sqrt(var + eps)
...
class ResFFT(nn.Module):
    ...
    def forward(self, x):
        ...
        x_fft = x_fft.permute(0, 1, 4, 2, 3, 5).contiguous()  # (batch, c, 2, h, w/2+1)
        ...
        x_fft_conv = x_fft_conv.view((batch, -1, 2,) + x_fft_conv.size()[2:]).permute(
            0, 1, 3, 4, 2, 5).contiguous()  # (batch,c, h, w/2+1, 2)
